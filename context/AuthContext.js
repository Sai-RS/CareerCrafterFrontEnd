...
export const AuthProvider = ({ children }) => {
  ...
  const resetPassword = async ({ email, new_password }) => {
    try {
      ...
      const res = await axios.post(
        `${process.env.API_URL}/api/resetpassword/`,
        {
          email,
          new_password,
        }
  ...
  /* // Reset Password 
  const resetPassword = async (newPassword,access_token) =>{
    try{
        const res = await axios.put(`${process.env.API_URL}/api/resetpassword/`,{new_password:newPassword},{
          headers:{
            Authorization:`Bearer ${access_token}`,
          },
        });
